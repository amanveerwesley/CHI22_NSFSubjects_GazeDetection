...
filenames = find_mp4_filenames("C:/Users/cpladmin/Desktop/NSFMath/Pilot_CSV_WorkingCopy/FACS/Input")

for name in filenames:
	p_id = name[:4]
	sess = name.split("_")[1]
	
	newmp4 = p_id + '_' + sess + '_FACS+Gaze.mp4'
	newcsv = p_id + '_' + sess + '_FACS+Gaze.csv'
	# Input video file
	cap = cv2.VideoCapture("C:/Users/cpladmin/Desktop/NSFMath/Pilot_CSV_WorkingCopy/FACS/Input" + name)
	# Output mp4
	...
	while(True):
		...
		for (x,y,w,h) in faces_F:
			...
			if (emotion_strength > .69):
				...
				cv2.putText(img, output, (halfway, int(y)), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 3)
